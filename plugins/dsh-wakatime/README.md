...
When the plugin is installed in a web profile, restart DSH and open **Settings → Plugins → WakaTime**. The page follows the official dashboard structure: activity overview, AI Coding, Projects/Categories, models, editors, languages, operating systems, machines, AI-versus-human trends, and project cards with AI details. The project view adds today's project/language/category breakdown, while AI and Insights provide prompt, token, model, weekday, and daily AI-share detail. Insights defaults to the last year and uses WakaTime's read-only `stats`, `days`, `ai_days`, and `weekdays` endpoints for long-range heatmaps; it does not use team, billing, or other paid-only APIs.
